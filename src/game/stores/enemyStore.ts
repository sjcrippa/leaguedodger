...
export const useEnemyStore = create<EnemyStore>((set, get) => ({
  ...
  spawnRandomEnemy: () => {
    ...
    if (isSpawning || isPaused || isGameOver || totalEnemies >= enemiesPerLevel) {
      return;
    }
  ...
  updateEnemies: playerPosition => {
    ...
    updatedEnemies.forEach(enemy => {
      ...
      updatedEnemies.forEach(otherEnemy => {
        ...
        if (distance < config.minEnemyDistance) {
          ...
          const awayFromOther = new Vector3()
            ...
            .normalize()
            .multiplyScalar(
              Math.pow((config.minEnemyDistance - distance) / config.minEnemyDistance, 3)
            ); // Cubic force
      ...
      if (
        enemyProjectilesEnabled &&
        distanceToPlayer <= config.attackRange &&
        timeSinceLastAttack >= config.attackCooldown
      ) {
        enemy.lastAttackTime = now;

        const projectileStart = enemy.position
          .clone()
          .add(directionToPlayer.clone().multiplyScalar(1.5));

        addProjectile(projectileStart, directionToPlayer, "enemy", config.projectileSpeed);
      }
    });
